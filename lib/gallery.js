...
import { Photos, download, downloadAllZip, fileNameOf } from './photos.js';
...
async function loadModels() {
  ...
  $('mdTable').style.display = list.length ? '' : 'none';
  ...
  list.forEach(m => {
    ...
    tr.innerHTML =
      ...
      '<td>' + T(SOURCE_LABELS[m.source] || m.source || '') + '</td>' +
      '<td>' + esc((m.classes || []).join(', ')) + '</td>' +
      '<td class="num">' + total + '</td>' +
      '<td class="num">' + (m.accuracy != null ? Math.round(m.accuracy * 100) + '%' : '-') + '</td>' +
      '<td class="num">' + fmtDate(m.createdAt) + '</td>';
    ...
    mk('fa-graduation-cap', '이어서 배우기', '', () => {
      location.href = 'train.html?load=' + encodeURIComponent(m.name);
    });
    ...
    mk('fa-pen', '이름 바꾸기', '', async () => {
      ...
      try {
        await Store.rename(m.name, to.trim());
        toast(T('이름을 바꿨어요'));
        loadModels();
      } catch (e) {
